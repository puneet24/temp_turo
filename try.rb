	...
	@all_states.each do |key,value|
		value.each do |city_url|
			...
			while i < count_of_vehicles.to_i do
				...
				# puts "price :- #{price}"
				# puts "*"*30
				query_form = 'INSERT INTO wp_data(state,city,owner_name,make_and_model,price,model_year,description,img_links) values("' + state_obj + '","' + city_obj + '","' + owner + '","' + make_and_model + '",' + price +  ',' + year + ',"' + description + '",' + image_urls + ')'
				#puts query_form 
